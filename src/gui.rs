//! cred-gui -- egui frontend for the credd credential daemon.
//!
//! Connects to credd on CREDD_URL (default http://localhost:4400) using
...
// ---------------------------------------------------------------------------
// API types (matches credd response shapes)
// ---------------------------------------------------------------------------
                ...
                UiCmd::LoadList => {
                        ...
                        .send()
                    {
                        Ok(r) if r.status().is_success() => {
                            match r.json::<Vec<SecretListItem>>() {
                                Ok(items) => WorkerMsg::List(items),
                    ...
                    match client.get(&url).bearer_auth(&owner_key).send() {
                        Ok(r) if r.status().is_success() => {
                            match r.json::<SecretValueRaw>() {
                                Ok(sv) => {
                                    // Extract a display string from the value JSON
...
/// Extract a human-readable display string from a SecretValue JSON object.
...
struct CredApp {
    ...
    pending_reveal: std::collections::HashSet<(String, String)>,
    filter: String,
    status: String,
    loading: bool,
    add: AddDialog,
...
impl CredApp {
    fn new(cmd_tx: Sender<UiCmd>, msg_rx: Receiver<WorkerMsg>) -> Self {
        ...
        Self {
            ...
            pending_reveal: std::collections::HashSet::new(),
            filter: String::new(),
                                ...
                                .show(ui, |ui| {
                                    for entry in entries.iter() {
                                        ...
                                        if let Some(plain) = self.revealed.get(&rk) {
                                            ...
                                            ui.label(&entry.redacted_preview);
